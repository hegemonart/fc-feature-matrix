...
|---|---|---|---|---|---|
| Language switcher in header | A visible control to change language rendered in the header | Lets users switch site language from the header | A | +1 | −8 |
| Login / account | A login link, login button, or account icon rendered in the header | Entry point for user account, sign-in, or profile | A | +1 | −8 |
...
| Shop shortcut in header | A top-level nav item linking to the club store | Top-nav link sending users straight to the club store | B | +2 | −5 |
| Tickets shortcut in header | A top-level nav item linking to tickets | Top-nav link sending users straight to tickets | B | +2 | −5 |
...
| Secondary editorial strip below hero | A row of 2+ editorial cards rendered within 200px below the hero area | Row of editorial cards just under the hero for quick content access | A | +1 | −8 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Next-match block | A block showing the next scheduled fixture with at least one of: date, time, or opponent | Shows the upcoming match so fans know when the team plays next | A | +1 | −8 |
| Next-match block is feature-rich | The next-match block contains 3+ of the following: live countdown, 2+ upcoming fixtures, opponent crest, competition logo, ticket CTA, broadcaster info, add-to-calendar button | Next-match block adds extras like countdown, tickets, or broadcaster info | C | +5 | −3 |
...
| Live match indicator | A badge, card, or banner indicating a currently-live match | Visible signal when a match is happening right now | C | +5 | −3 |
...
| Matchday experience info block | A block with matchday logistics (transport, food, fan zone, or gates) | Practical info about attending a match: transport, food, gates, fan zone | C | +5 | −3 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Dedicated news section | A homepage section labelled News or equivalent containing 2+ news items | A section of the homepage clearly devoted to news stories | A | +1 | −8 |
...
| Homepage video block | A block dedicated to video content, rendered outside the news section | A dedicated area for video content, separate from news | B | +2 | −5 |
...
| Documentary promo block | A block, modal, or upsell specifically promoting a documentary | Specific promo for a documentary film, often with a paywall CTA | C | +5 | −3 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Tickets block on homepage | A block promoting ticket sales (not just a header nav link) | A real block selling or promoting tickets, not just a nav link | C | +5 | −3 |
| Hospitality block | A block with at least a brief description of hospitality packages (a link alone does not qualify) | Promotes premium hospitality packages with some descriptive content | C | +5 | −3 |
| Stadium tours block | A block promoting stadium tours (a nav entry alone does not qualify) | Promotes tours of the stadium with content, not just a nav link | C | +5 | −3 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Store block on homepage | A block dedicated to the club store (any size, must meet the block definition) | Any block dedicated to the official club store | B | +2 | −5 |
| Store block shows individual products with CTAs | The store block contains individual product cards or featured collections, each with its own buy/shop CTA | Store block features specific product cards, each with its own buy button | C | +5 | −3 |
| Member-only commerce block | A block promoting products or discounts exclusive to members | Products or discounts available only to signed-up members | C | +5 | −3 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Newsletter signup block | A block containing an email input, or a block dedicated to newsletter subscription with a CTA | An email capture block for joining the club's newsletter | B | +2 | −5 |
| Fan-club / free membership sign-up block | A block promoting sign-up to the club's official fan community or membership programme (a standalone CTA button does not qualify) | Promotes joining the club's free official fan community or membership | C | +5 | −3 |
| Paid membership / subscription plans block | A block with a CTA leading to paid sign-up or a plans list (e.g. "View plans", "See plans", "Become Premium", "Subscribe") | Promotes paid tiers like premium access, streaming, or subscription plans | C | +5 | −3 |
...
| Museum block | A block about the museum (a nav or footer link alone does not qualify) | A dedicated block promoting the club's museum | C | +5 | −3 |
...
| Women's team featured on homepage | Women's team content (news, tickets, or a block) appears anywhere on the homepage | Any women's team content visible on the homepage | B | +2 | −5 |
...
| In-content sponsor placement | A sponsor visibly placed within homepage content, outside the header and footer | Sponsor shown inside homepage content, not just header or footer | C | +5 | −3 |
| App store badges | Apple, Google, or AppGallery download badges rendered on the homepage | App Store, Google Play, or AppGallery download badges on the homepage | B | +2 | −5 |
| Club TV app promotion block | A block promoting the club's streaming/TV app with a subscribe or download CTA | Promotes the club's streaming app with a subscribe or download CTA | C | +5 | −3 |
...
| Loyalty points / rewards block | A block about a points or rewards programme (implied via generic membership does not qualify) | A points or rewards system promoted on the homepage | C | +5 | −3 |
...
| Feature | Qualifies as Yes if | Description | Tier | Weight if Yes | Weight if No |
|---|---|---|---|---|---|
| Social links in footer | Social media icons or links rendered in the footer | Social media icons or links in the footer area | A | +1 | −8 |
| Language / region selector in footer | A language or region switcher rendered in the footer | Language or region switcher placed in the footer | A | +1 | −8 |