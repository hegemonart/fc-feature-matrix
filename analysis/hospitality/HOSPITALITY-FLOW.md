...
**Version:** v1.1 (2026-04-28 — added `Detection` column for Plan 02-15 hybrid routing)
**Date:** 24 April 2026
...
- **Detection** — Plan 02-15 hybrid-routing tag, controls how the scanner decides presence:
  - `dom` — programmatically clear from the DOM. The scanner runs the registered rule in `scanner/vision/dom_detect.py::RULES` and skips the vision wave for this feature.
  - `visual` — pure visual quality / brand judgment. The scanner sends the screenshot to Opus + Sonnet (no DOM rule consulted).
  - `hybrid` — both signals are useful. DOM rule runs first; if it returns true, the vision wave is short-circuited at confidence ≥ 0.95. If it returns false (or no rule registered), vision takes over.
...
|-------------|------|-------------|------|-----------|--------|----------------|
| `package_tier_list` | Package Tier List | Landing page lists all packages with distinguishing copy in one view. | 1 | hybrid | O | All 10 gold sites; MCFC `/hospitality`; CHE `hospitality.chelseafc.com`. |
| `per_tier_landing_page` | Per-Tier Landing Page | Clicking a tier opens its own description landing page. | 1 | visual | O | MCFC `/hospitality/the-tunnel-club-premier`. |
| `dress_code_info` | Dress-Code Info | Explicit per-tier dress code (smart, smart-casual, formal). | 1 | dom | C | Seat Unique Trustpilot (Tier-A #1). |
| `tier_comparison_table` | Tier Comparison Table | Side-by-side feature matrix across tiers on a single page. | 2 | dom | O | F1 Paddock Club (Tier-A #3). |
| `tier_capacity_indicator` | Tier Capacity Indicator | Min/max guests or per-box headcount visible. | 2 | hybrid | O, W | F1 Legends Club; Seat Unique Trustpilot (Tier-A #1). |
| `upcoming_matches_pre_tier` | Upcoming Matches Pre-Tier | Package cards show which matches apply before tier-selection. | 2 | hybrid | O | TOT `/matchday-options/`. |
| `multi_occasion_tagging` | Multi-Occasion Tagging | Booking captures occasion (birthday / anniversary / corporate / stag) for tailored service. | 3 | dom | O, W | goseelearn family review (Tier-A #11); F1 Paddock Club analog. |
...
| `total_cost_preview_pre_submit` | Total Cost Preview Pre-Submit | Running total shown before enquiry submission. | 2 | hybrid | W | Baymard-style wish; voiced across Tier-A blogs. |
| `deposit_vs_full_payment` | Deposit Vs Full Payment | Deposit vs full payment terms shown pre-enquiry. | 2 | dom | C | Trustpilot surprise-terms complaints (Tier-A #1–3). |
| `corporate_invoice_billing` | Corporate Invoice Billing | VAT-invoice / PO / 30-day-term option for corporate buyers at enquiry stage. | 2 | dom | W | P1 Travel Trustpilot (Tier-A #4); sportspro.com B2B (Tier-C #19). |
| `vat_inclusive_toggle` | VAT Inclusive Toggle | Switch between inc-VAT and ex-VAT for corporate buyers. | 3 | dom | O | Some gold sites; F1 corporate flow. |

## 3. Food & Beverage

| Feature Key | Name | Description | Tier | Detection | Origin | Evidence/Notes |
|-------------|------|-------------|------|-----------|--------|----------------|
| `menu_preview` | Menu Preview | Representative menu shown on tier page (not "TBC"). | 1 | hybrid | O, W | F1 3-course sample; MCFC Tunnel Club Premier "five-course". |
| `allergen_info` | Allergen Info | Allergen info available pre-booking (not only on the night). | 1 | dom | C, W | Keith Prowse Trustpilot (Tier-A #2). |
| `vegetarian_options` | Vegetarian Options | Veggie / vegan option disclosure on menu preview. | 1 | dom | C | Keith Prowse Trustpilot (Tier-A #2). |
| `bar_type_indicator` | Bar Type Indicator | Complimentary / cash / premium-only bar clearly indicated. | 1 | dom | C | CHE Centenary cash-bar; TOT TripAdvisor "spirits at separate bar" (Tier-A #5, #7). |
| `chef_attribution` | Chef Attribution | Chef or restaurant partner explicitly credited. | 2 | dom | O | F1 "renowned chefs"; MCFC "award-winning executive chefs". |
| `kids_menu` | Kids Menu | Explicit kids menu or child-friendly hospitality indicator. | 2 | dom | W | goseelearn family review (Tier-A #11). |
| `meal_timing_visible` | Meal Timing Visible | Arrival / pre-match / post-match meal timing stated. | 2 | dom | O | CHE Platinum "2.5h prior"; TOT "3h pre / 2h post". |
| `post_match_service` | Post-Match Service | What stays open after final whistle. | 2 | dom | O | CHE Tambling "~1h after"; TOT "2h after final whistle". |

## 4. Match Selector UX

| Feature Key | Name | Description | Tier | Detection | Origin | Evidence/Notes |
|-------------|------|-------------|------|-----------|--------|----------------|
| `fixture_list_visible` | Fixture List Visible | Standard fixture list on match-selector step. | 1 | hybrid | O | Standard across all 10 gold sites. |
| `competition_filter` | Competition Filter | PL / UCL / Cup filter on selector. | 1 | dom | O | EPL + UCL clubs require this. |
| `sold_out_indicator` | Sold-Out Indicator | Explicit sold-out state on fixture rows. | 1 | hybrid | O | Common; missed by some (surfaces during crawl). |
| `multi_match_bundle_selector` | Multi-Match Bundle Selector | Bundle several fixtures in one flow. | 2 | hybrid | O | FIFA 2026; Real Madrid seasonal VIP. |
| `opponent_filter` | Opponent Filter | Search / filter by opponent. | 3 | dom | O | F1 has via circuit; rare in football. |
| `availability_heatmap` | Availability Heatmap | Availability calendar view (vs list). | 3 | visual | W | Reddit / blog wish-lists (Tier-B #17, #18); no football club has it. |

## 5. Enquiry Friction

| Feature Key | Name | Description | Tier | Detection | Origin | Evidence/Notes |
|-------------|------|-------------|------|-----------|--------|----------------|
| `enquiry_form_field_count_le_7` | Enquiry Form Field Count ≤ 7 | Form ≤ 7 fields (Baymard: 13→7 lifts completion ~15%). | 1 | dom | C, W | Hospitality forms typically ~12+ (Tier-A blog reviews #8–10). |
...
| `merchandise_voucher` | Merchandise Voucher | Merchandise voucher included. | 3 | dom | O, W | Some clubs bundle (TOT, MCFC). |
| `transport_package_bundling` | Transport Package Bundling | Flights / hotels / train bundled into hospitality package (broker-adjacent). | 3 | dom | O | Real Madrid Emirates Skywards (Tier-A #6); P1 Travel pattern (Tier-A #4). |

## 7. Post-Booking Comms

| Feature Key | Name | Description | Tier | Detection | Origin | Evidence/Notes |
|-------------|------|-------------|------|-----------|--------|----------------|
| `booking_change_policy_visible` | Booking Change Policy Visible | Change / cancellation policy visible pre-submit. | 1 | dom | C | Seat Unique date-change-refund-refused complaint (Tier-A #1). |
| `seat_number_lead_time` | Seat Number Lead Time | When seat numbers are communicated (days before). | 1 | dom | C | Seat Unique "4–5 days before" complaint (Tier-A #1). |
| `fixture_change_notification` | Fixture Change Notification | Schedule-change notification policy stated. | 1 | dom | C | Eventmasters complaint (Tier-A #3). |
| `dietary_preferences_capture` | Dietary Preferences Capture | Dietary prefs captured at booking (not re-asked on the night). | 1 | dom | C, W | Allergen complaints imply this (Tier-A #2). |
| `cancellation_refund_window` | Cancellation Refund Window | Cancellation-refund window disclosed (dates & amounts refundable vs policy-existence). | 1 | dom | C | Seat Unique Trustpilot (Tier-A #1) — distinct from change-policy row. |
| `group_host_contact` | Group Host Contact | Host contact or allocation protocol for group bookings. | 2 | dom | C | Eventmasters table-allocation complaint (Tier-A #3). |
...
| Feature Key | Name | Description | Tier | Detection | Origin | Evidence/Notes |
|-------------|------|-------------|------|-----------|--------|----------------|
| `confirmation_page_clarity` | Confirmation Page Clarity | What the form PROMISES will happen post-submit (copy-only). | 1 | hybrid | O | Varies widely across 10 gold sites. |
| `saved_booking_in_account` | Saved Booking In Account | Past bookings visible in account area (login-gated clubs). | 2 | visual | O | Login-gated hospitality (D-12). |
| `receipt_download` | Receipt Download | Self-serve receipt / VAT invoice download. | 2 | dom | O | Corporate buyer need. |
...
## Detection Mode Distribution (Plan 02-15 Wave D)

| Mode | Count | % | Examples |
|------|------:|--:|----------|
| `dom` | 33 | 60% | Form input counts, price patterns, keyword/text rules, tel: links, ld+json checks. |
| `visual` | 3 | 5% | `per_tier_landing_page`, `availability_heatmap`, `saved_booking_in_account` (require visual-quality judgment without programmatic signal). |
| `hybrid` | 19 | 35% | DOM rule narrows the question; vision confirms. e.g. `package_tier_list`, `menu_preview`, `live_chat_availability`. |
| **Total** | **55** | **100%** | — |

These tags drive `scanner/vision/judge.py::two_judge` routing in Wave E:
- `dom` → DOM rule only; vision skipped → ~$0 per feature.
- `hybrid` → DOM rule first; vision only on miss → ~40% vision-cost saving on those rows.
- `visual` → vision wave runs unchanged.

Projected cost (vs Plan 02-11 retail equivalent of $5.50–6.50): **$1.50–2.50** for a re-run of the same 16 PNGs (~60% reduction).
...
`scanner/config/areas.json` for the scanner pipeline. Detection-mode column
added by Plan 02-15 Wave D (2026-04-28) for hybrid DOM+vision routing.*
